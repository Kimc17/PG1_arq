R= {'add' : ["000"], 'mul' : ["001"],'lar' : ["010"], 'and' :["011"],
       'sub' : ["100"]}
...
codigo=""
f = open("codigo.txt", "r")
f2 = open("compilado_codigo.data", "w")
lineas = f.readlines()
print(lineas)
Ra= 0
Rb=0
...
for i in lineas:
    ...
    if cad.find(":") != -1:
        print("etiqueta")
    elif (cad[0:3]  == 'add' or cad[0:3]  == 'mul' or cad[0:3]  == 'lar' or  cad[0:3]  == 'and' or cad[0:3]  == 'sub'):
        for j in R:
            if cad[0:3] == j:
                codigo += "00000"
                Rd= str(bin(int(cad[4]))[2:])
                while len(Rd)<4:
                    Rd ="0"+ Rd
                codigo += Rd
                #print(Rd)
                
                Ra= str(bin(int(cad[6]))[2:])
                while len(Ra)<4:
                    Ra ="0"+ Ra
                codigo += Ra
                #print(Ra)
                
                Rb= str(bin(int(cad[8]))[2:])
                while len(Rb)<4:
                    Rb ="0"+ Rb
                codigo += Rb
                codigo += R[j][0]
